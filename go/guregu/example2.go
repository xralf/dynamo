...
func UpdateTable() {
	...
	for i := 0; i < 10; i++ {
		...
		// 	Widget{Id: 102, Time: time.Now(), Msg: "aaa", Count: rand.Intn(10)},
		// 	Widget{Id: 103, Time: time.Now(), Msg: "bbb", Count: rand.Intn(10), SecretKey2: "foo"},
		// 	Widget{Id: 104, Time: time.Now(), Msg: "aaa", Count: rand.Intn(10)},
		// 	Widget{Id: 105, Time: time.Now(), Msg: "aaa", Count: rand.Intn(10)},
		// }
	...
	// Use placeholders in filter expressions.
	{
		...
		const minCount = 6
		if err := table.
			Get("idType", models.IdOrder).
			Index("idType-page-index").
			AllWithLastEvaluatedKey(&filtered); err != nil {
			log.Fatal(err)
		}
		fmt.Printf("Items with count >= %d:\n", minCount)
		for i, item := range filtered {
